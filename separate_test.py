...
parser.add_argument('--train_dir', type=str, required=True, help='Training Weights Directory')
parser.add_argument('--eval_dir', type=str, required=True, help='Output of this program')
parser.add_argument('--epoch', type=str, required=True, help='Epoch Number Weights')
...
checkpoint = torch.load(str(args.train_dir)+'/checkpoint_'+str(args.epoch)+'.pt')
...
for item in testdata.test_samples():
	...
	torchaudio.save(str(args.eval_dir)+"/testsep1_"+str(i)+"_.wav", sep1wav, 16000)
	torchaudio.save(str(args.eval_dir)+"/testsep2_"+str(i)+"_.wav", sep2wav, 16000)
	torchaudio.save(str(args.eval_dir)+"/origmix_"+str(i)+"_.wav", unified_mix.squeeze(dim = 0), 16000)
	torchaudio.save(str(args.eval_dir)+"/origsep1_"+str(i)+"_.wav", mix[0,:,:], 16000)
	torchaudio.save(str(args.eval_dir)+"/origsep2_"+str(i)+"_.wav", mix[1,:,:], 16000)
